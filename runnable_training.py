...
folder_name = "qutrit\\"
state = 1/9 * qt.tensor(qt.identity(3), qt.identity(3))
ket = (qt.tensor(qt.basis(3,0), qt.basis(3,0)) \
    + qt.tensor(qt.basis(3,1), qt.basis(3,1)) \
        + qt.tensor(qt.basis(3,2), qt.basis(3,2))).unit()
state = 0.5 * 1/9 * qt.tensor(qt.identity(3), qt.identity(3)) + 0.5 * qt.ket2dm(ket)
...
minima, history = train_generator(model, create_generator(state, dim=3), save=True,
                        save_name=folder_name + 'qutrit_5_noise.h5', loss=customLoss_multiple, steps=100)
