...
def build_model_comm_polar():
    ...
    activ2 = 'softmax'
    activ3 = config.activation_func_comm
    # 4 numbers (two 3D vectors in polar) plus hidden variable as inputs.
    inputTensor = Input((4+number_of_LHV,))

    # Group input tensor according to whether alpha, beta or gamma hidden variable.
    group_alpha = Lambda(lambda x: x[:, 0:2], output_shape=((2,)))(inputTensor)
    group_beta = Lambda(lambda x: x[:, 2:4], output_shape=((2,)))(inputTensor)
    group_LHV = Lambda(lambda x: x[:, 4:4+number_of_LHV],
                       output_shape=((number_of_LHV,)))(inputTensor)
